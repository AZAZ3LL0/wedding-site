...
// Everything the organizer's message shows, read at send time (tech.md §5).
...
export async function findRsvpNotice(db: Db, guestId: string): Promise<RsvpNotice | null> {
	const [row] = await db
		.select({
			firstName: guests.firstName,
			lastName: guests.lastName,
	...
	const { firstName, lastName, telegramUsername, partyTitle, ...answer } = row;
	return {
		guest: { firstName, lastName, partyTitle, telegramUsername },
		answer,
		companion: await findCompanion(db, guestId)
	};
}
